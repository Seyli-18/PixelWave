...
export async function getTopScores(gameId, classId = '') {
  try {
    let q;
    // On récupère plus d'entrées (50) pour pouvoir dédupliquer par pseudo
    if (classId) {
      q = query(
    ...
    // Déduplication : un pseudo → uniquement son meilleur score
    const seen = new Map();
    ...
    // Retrier après dédup et limiter à 10
